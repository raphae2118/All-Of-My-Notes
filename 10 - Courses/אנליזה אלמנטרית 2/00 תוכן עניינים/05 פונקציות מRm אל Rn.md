...
$$
נטיל את הצורה למישור$x,y$ בשביל לקבל תחום הגדרה $D$ 
![[Pasted image 20260708142234.png]]
כאשר 
$$
	\Omega = \{ (x,y,z),| (x,y) \in D \quad \alpha_{1}(x,y) \leq z \leq \alpha_{2}(x,y) \}
$$
אזי האינטגרל
$$
	I = \iint_{D} \int^{\alpha_{2}(x,y)}_{\alpha_{1}(x,y)} f(x,y,z) dz dxdy
$$
נראה [[דוגמא לאינטגרל משולש]]
שמה התבקשנו לחשב אינטגרל משלוש בתחום של טטראדרון מעל $\mathbb{R}^3$ 
בשביל לחשב את הנפח הצגנו [[עקרון קאוואליירי]]
בדוגמא לעיל התבקשנו למצוא את 
$$
		\iiint_{\Omega} dV, \; \iiint_{\Omega}z \;dV
$$
מסתבר שזה קשור אל [[מיקום מרכז המסה]].
![[מיקום מרכז המסה#הגדרה מאנליזה]]

הרחבנו את [[עקרון קאוואליירי]] גם בעבור גוף סיבוב וראינו [[דוגמא נפח גוף סיבוב מעיקרון קאוואליירי]]
ניכר כי צריך לבחור בקפידה את סדר האינטגרציה על מנת להגיע לאינטגרל שהוא הגיוני.
לדוגמא השאלה הבאה מתרגיל הבית 
![[Pasted image 20260708192208.png]]
ראשית נרצה לכתוב את $\Omega$ כקבוצה נשים לב כי ניתן להבין חסמים עבור $y$ 
$$
	0 \leq y \leq 2
$$
נבטא באמצעותו את שאר החסמים 
$$
	\Omega = \{ (x,y,z)| 0 \leq y \leq 2,\quad 0\leq z \leq 2-y,\quad 0\leq x \leq 4-y^2 \}
$$
התחום של $y$ הוא הכי פשוט אז הוא יהיה האינטגרל החיצוני ביותר שלנו.
$$
	V = \int^2_{0}\iint \dots dy
$$
כעת יש לנו שני אופציות לשני האינטגרלים הפנימיים יותר או
$$
	dxdz
$$
או 
$$
	dzdx
$$
כאן סדר האינטגרציה לא משנה כל כך.
